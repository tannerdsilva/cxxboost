...
class PackageDescriptionTargetItemNameScanner:SyntaxVisitor {
	...
	override func visit(_ node:LabeledExprSyntax) -> SyntaxVisitorContinueKind {
		...
		logger[metadataKey:"node_type"] = "LabeledExprSyntax"
		logger.trace("visiting labeled expression list...")
		guard node.label?.text == "name" else {
			logger.debug("skipping labeled expression list because it is not a `name` label.")
			return .skipChildren
		}
		if let memberAccess = node.expression.as(MemberAccessExprSyntax.self), let declBaseName = memberAccess.base?.as(DeclReferenceExprSyntax.self) {
			...
			if base.hasPrefix(self.expectedTargetNamePrefix) {
				...
				logger.debug("skipping labeled expression list because it does not have the expected target name prefix.")
			}
			...
			logger.debug("skipping labeled expression list because it does not have a valid expression.")
		}
		return .skipChildren
	}
}
...
class PackageDescriptionTargetsArrayScanner:SyntaxVisitor {
	...
	init(log:Logger, memberAccessName:String, _ whenFound:@escaping((LabeledExprListSyntax) -> Void)) {
		self.whenFound = whenFound
		self.logger = log
		super.init(viewMode:.sourceAccurate)
	}
	...
	override func visit(_ node:FunctionCallExprSyntax) -> SyntaxVisitorContinueKind {
		...
		guard let declRefExpr = node.calledExpression.as(MemberAccessExprSyntax.self), declRefExpr.declName.baseName.text == "target" else {
			logger.debug("skipping function call expression because it is not an `Array`.")
			return .skipChildren
		}
